/**
 * Deterministic fixtures for Agent Framework for Java.
 *
 * <p>Everything in this package must be reproducible. No wall-clock reads, no random seeds, and no
 * network access, because golden scenarios compare exact event sequences across runs.
 *
 * <p>Requirements for this package live in {@code docs/requirements/11-operations.md}.
 */
package io.github.hellices.agentframework.testkit;
