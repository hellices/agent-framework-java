 ...
 * single request can survive a replacement by being sent again on the new generation, but a page
 * cannot: its cursor is opaque state that belongs to the session that issued it, and continuing
 * someone else's pagination on a new session is either rejected or, worse, answered from a
 * different position. The operation that caused the replacement is kept as the cause so that a
 * reader which cannot restart still reports something true.
 */
