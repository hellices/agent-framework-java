...
| ID | 요구사항 | 등급 | 단계 |
| --- | --- | --- | --- |
| INT-001 | 공개 확장점은 책임별 타입 인터셉터로 고정한다 | 필수 | MVP |
| INT-002 | 인터셉터는 DI·트랜잭션·보안·AOP를 재구현하지 않는다 | 필수 | MVP |
| INT-003 | 등록 순서가 전처리 순서와 감싸기 방향을 결정한다 | 필수 | MVP |
| INT-004 | 인터셉터 컨텍스트는 명시적이며 가변적이다 | 필수 | MVP |
| INT-005 | 지원하지 않는 seam 설치는 즉시 실패한다 | 필수 | MVP |
| INT-006 | `next`를 호출하지 않으면 단축 종료한다 | 필수 | MVP |
| INT-007 | 후처리에서 결과를 대체할 수 있다 | 필수 | MVP |
| INT-008 | 도구 예외 처리 규칙은 실행 전후를 구분한다 | 권장 | Core+ |
| INT-009 | 세션 상태 기계는 코어가 소유하고 인터셉터는 관여 지점만 가진다 | 필수 | Core+ |
| INT-010 | 다중 seam 기능은 opaque bundle로만 배포한다 | 권장 | Core+ |
| INT-011 | 도구 집합 변경은 다음 반복에만 반영한다 | 권장 | Core+ |
| INT-012 | 스트리밍 재작성은 finalization 경계 안에서만 허용한다 | 필수 | Core+ |
| INT-013 | 취소는 모든 인터셉터와 컴팩션으로 전파한다 | 필수 | MVP |
| INT-014 | 컴팩션은 도구 호출과 도구 결과의 원자성을 보존한다 | 필수 | Core+ |
| INT-015 | 컴팩션은 증분 업데이트에서도 식별자와 계수를 안정적으로 유지한다 | 필수 | Core+ |
| INT-016 | 컴팩션 내부 상태는 명시적 index와 trace metadata로 표현한다 | 필수 | Core+ |
| INT-017 | 컴팩션 시작 조건과 종료 조건을 분리한다 | 권장 | Core+ |
| INT-018 | group 기반과 turn 기반 sliding 정책을 분리한다 | 권장 | Core+ |
| INT-019 | tool-result compaction은 요약으로 대체하되 원문을 다시 노출하지 않는다 | 권장 | Core+ |
| INT-020 | summarization compaction은 실패 시 원본을 복구한다 | 필수 | Core+ |
| INT-021 | compaction provider는 실행 전 projection과 선택적 저장 후 훅을 제공한다 | 권장 | Core+ |
| INT-022 | 로깅과 텔레메트리 인터셉터는 민감 데이터 노출을 opt-in으로 둔다 | 권장 | Optional |
