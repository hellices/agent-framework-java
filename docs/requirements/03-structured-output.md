...
| ID | 요구사항 | 등급 | 단계 |
| --- | --- | --- | --- |
| OUT-001 | 구조화 출력은 명시적으로 요청한다 | 필수 | MVP |
| OUT-002 | 타입 검증 경로와 JSON 전용 경로를 함께 지원한다 | 필수 | MVP |
| OUT-003 | 비객체 목표 타입은 래퍼 스키마로 요청한다 | 필수 | MVP |
| OUT-004 | response format 우선순위를 고정한다 | 필수 | MVP |
| OUT-005 | 네이티브 지원 여부는 최선 노력 계약으로 본다 | 필수 | MVP |
| OUT-006 | 타입 값 파싱은 응답 접근 시점에 수행한다 | 필수 | MVP |
| OUT-007 | 구조화 파싱 대상은 마지막 assistant 텍스트 본문이다 | 필수 | MVP |
| OUT-008 | 검증 오류와 JSON 파싱 오류를 구분한다 | 필수 | MVP |
| OUT-009 | 비어 있거나 null인 structured payload는 명시적으로 실패한다 | 필수 | MVP |
| OUT-010 | 유효한 JSON 텍스트면 네이티브 미지원이어도 폴백 파싱한다 | 필수 | MVP |
| OUT-011 | 래퍼가 기대됐는데 bare JSON이 오면 원본 JSON으로 재시도한다 | 권장 | Core+ |
| OUT-012 | 구조화 스트리밍 값은 최종 응답에서만 읽는다 | 필수 | Core+ |
