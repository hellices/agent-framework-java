...
| ID | 요구사항 | 등급 | 단계 |
| --- | --- | --- | --- |
| AGT-001 | 공개 진입점은 `Agent` 하나로 통일한다 | 필수 | MVP |
| AGT-002 | 모든 에이전트는 안정적인 식별자를 가진다 | 필수 | MVP |
| AGT-003 | 실행과 스트리밍 진입점을 분리한다 | 필수 | MVP |
| AGT-004 | 스트리밍만 소비해도 실행이 완결된다 | 필수 | MVP |
| AGT-005 | 취소는 명시적 인자로 전달한다 | 필수 | MVP |
| AGT-006 | 에이전트는 세션 호환성만 검증한다 | 필수 | MVP |
| AGT-007 | 데코레이터로 에이전트를 감쌀 수 있다 | 필수 | MVP |
| AGT-008 | 실행 컨텍스트를 명시적으로 전달한다 | 필수 | MVP |
| AGT-009 | 모델 호출은 `ChatClient` 포트로 분리한다 | 필수 | MVP |
| AGT-010 | 선택적 모델 기능은 별도 인터페이스로 노출한다 | 필수 | MVP |
| AGT-011 | 요청 옵션은 공급자 중립 키를 1차 계약으로 둔다 | 필수 | MVP |
| AGT-012 | 옵션 병합 우선순위를 고정한다 | 필수 | MVP |
| AGT-013 | 실행 단위로 모델 클라이언트를 교체할 수 있다 | 권장 | Core+ |
| AGT-014 | 이어받기 실행에 새 입력을 함께 줄 수 없다 | 필수 | Core+ |
| AGT-015 | 스트리밍 조각만으로 최종 응답을 복원한다 | 필수 | MVP |
| AGT-016 | 서비스가 이력을 저장하면 로컬 이력을 중복 저장하지 않는다 | 필수 | Core+ |
...
**요구사항.** 취소 신호는 공개 실행 API의 명시적 인자여야 한다. 스레드 로컬이나 암묵적
실행 컨텍스트에서 취소를 읽지 않는다.
...
**판단.** .NET 방식을 택한다. Java에는 asyncio에 대응하는 표준 암묵 취소가 없다. 취소를
명시적 인자로 두면 호스트가 요청 시간 초과를 그대로 전파할 수 있고, 취소 전파 누락이
컴파일 시점 또는 리뷰에서 드러난다.
...
## AGT-011 요청 옵션은 공급자 중립 키를 1차 계약으로 둔다

**요구사항.** 온도, 최대 토큰처럼 공통된 요청 옵션은 코어 타입으로 정의하고, 공급자 고유
옵션은 분리된 영역에 담아야 한다. 공급자 고유 키가 공통 옵션과 같은 평면에 섞이지 않는다.
...
**판단.** 두 원본 모두 공통 옵션을 1차 계약으로 둔다. Java도 같은 구조를 택하되, 공급자
고유 옵션을 분리해 어댑터 교체 시 어떤 설정이 이식되지 않는지 즉시 드러나게 한다.
