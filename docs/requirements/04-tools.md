...
| ID | 요구사항 | 등급 | 단계 |
| --- | --- | --- | --- |
| TOOL-001 | 도구는 코어 정의 타입으로 표현한다 | 필수 | MVP |
| TOOL-002 | 도구 실행 루프는 Java 코어가 단독 소유한다 | 필수 | MVP |
| TOOL-003 | 입력 스키마는 추론하되 명시 스키마가 이긴다 | 필수 | MVP |
| TOOL-004 | 컨텍스트 인자는 스키마 밖에서 주입한다 | 필수 | MVP |
| TOOL-005 | 모든 호출 경로에서 인자를 검증한다 | 필수 | MVP |
| TOOL-006 | 선언 전용 도구와 추가 도구는 로컬 실행하지 않는다 | 필수 | Core+ |
| TOOL-007 | 도구 선택은 `ToolMode`로 고정한다 | 필수 | MVP |
| TOOL-008 | 실행 옵션이 기본 옵션을 이긴다 | 필수 | MVP |
| TOOL-009 | 호출 설정은 정규화된 타입과 안전 기본값을 가진다 | 필수 | MVP |
| TOOL-010 | 반복 제한과 호출 예산의 의미를 고정한다 | 필수 | MVP |
| TOOL-011 | 호출 예산은 배치 뒤 best-effort로 차단한다 | 필수 | MVP |
| TOOL-012 | 병렬 실행은 실행 가능 배치만 다루고 순서를 보존한다 | 필수 | MVP |
| TOOL-013 | 도구 결과는 `Content` 목록으로 정규화한다 | 필수 | MVP |
| TOOL-014 | 도구 정의와 런타임 카운터를 분리한다 | 권장 | Core+ |
| TOOL-015 | 스트리밍도 같은 도구 루프를 탄다 | 필수 | MVP |
| TOOL-016 | 승인 요청과 응답은 코어 콘텐츠다 | 필수 | Core+ |
| TOOL-017 | 승인 응답은 원래 요청에만 결합한다 | 필수 | Core+ |
| TOOL-018 | 승인 거부는 합성 종료 결과를 남긴다 | 필수 | Core+ |
| TOOL-019 | 상시 승인은 정확한 인자와 호스트 경계로 매칭한다 | 필수 | Core+ |
| TOOL-020 | 승인 큐는 세션 위에서 하나씩 진행한다 | 필수 | Core+ |
| TOOL-021 | 자동 승인 연쇄는 상한으로 끊는다 | 권장 | Core+ |
