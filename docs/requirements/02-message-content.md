...
| ID | 요구사항 | 등급 | 단계 |
| --- | --- | --- | --- |
| MSG-001 | 코어가 대화 타입을 직접 소유한다 | 필수 | MVP |
| MSG-002 | 역할은 알려진 값과 사용자 정의 값을 함께 허용한다 | 필수 | MVP |
| MSG-003 | 입력을 메시지 목록으로 정규화한다 | 필수 | MVP |
| MSG-004 | 텍스트 투영은 텍스트 콘텐츠만 본다 | 필수 | MVP |
| MSG-005 | 코어가 기본 멀티모달 콘텐츠 종류를 제공한다 | 필수 | Core+ |
| MSG-006 | URI와 바이너리 콘텐츠를 생성 시 검증한다 | 필수 | Core+ |
| MSG-007 | 추가 속성과 원시 표현을 보존한다 | 필수 | MVP |
| MSG-008 | 메시지 출처는 콘텐츠와 분리해 태깅한다 | 필수 | Core+ |
| MSG-009 | 선두 지시문은 중복 삽입하지 않는다 | 권장 | Core+ |
| MSG-010 | 사용량은 표준 필드와 확장 필드를 함께 가진다 | 필수 | MVP |
| MSG-011 | 응답과 업데이트는 같은 메타데이터 축을 공유한다 | 필수 | MVP |
| MSG-012 | 업데이트 시퀀스에서 최종 응답을 복원한다 | 필수 | MVP |
| MSG-013 | 스트림 래퍼는 최종 응답과 변환 훅을 제공한다 | 권장 | Core+ |
