...
- 세션·저장소(`SES01`, `SES02`)와 이력·컨텍스트(`CTX01`, `CTX02`)는 코어 실행 경로의 일부로 모두 채택 `필수`다.

## 요약

| ID | 요구사항 | 채택 | 등급 | 단계 |
| --- | --- | --- | --- | --- |
| SES-001 | 공개 세션은 로컬 ID와 서비스 핸들을 분리한다 | 필수 | 필수 | MVP |
| SES-002 | 서비스 대화 식별자는 권한 경계가 아니다 | 필수 | 필수 | MVP |
| SES-003 | 세션이 영속 대화 상태의 단일 기준이다 | 필수 | 필수 | MVP |
...
| SES-011 | 실행마다 독립적인 `SessionContext`를 만든다 | 필수 | 필수 | MVP |
| SES-012 | 컨텍스트 제공자는 세션 네임스페이스 안에서 상태를 관리한다 | 필수 | 필수 | Core+ |
| SES-013 | 이력 제공자는 선택적 적재·저장 플래그를 노출한다 | 필수 | 필수 | Core+ |
| SES-014 | 기본 인메모리 이력 주입 조건을 엄격히 제한한다 | 필수 | 권장 | Core+ |
| SES-015 | 서비스 호출 단위 이력 저장은 명시적 옵션이다 | 필수 | 권장 | Core+ |
| SES-016 | 로컬 이력 모드는 기존 서비스 대화와 혼용하지 않는다 | 필수 | 필수 | Core+ |
| SES-017 | 메시지 주입 큐는 세션에 붙고 대화 연속성을 유지한다 | 필수 | 필수 | Core+ |
| SES-018 | 파일 이력은 세션 스냅샷과 분리된 채널에 저장한다 | 필수 | 권장 | Optional |
| SES-019 | 외부 메모리는 명시적 범위와 비신뢰 문맥으로 연동한다 | 필수 | 권장 | Optional |
| SES-020 | 호스팅 격리는 별도 사용자 컨텍스트가 맡는다 | 필수 | 필수 | Hosting |
